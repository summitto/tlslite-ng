...
class AESGCM(object):
    ...
    def __init__(self, key, implementation, rawAesEncrypt):
        ...
        print("h:", h_bytes.hex())
        print("h_reverse: ", reverseEndianness(h_bytes.hex()))
        ...
        h_table = [
            h,
            bytesToNumber(bytes.fromhex("ffa5f9409bf15bb0831967be333fc74f")),
            bytesToNumber(bytes.fromhex("68009948c9489f9643d26dbb4e0a2005")),
            bytesToNumber(bytes.fromhex("c10f1f4418e71843df1333422fff6694")),
            bytesToNumber(bytes.fromhex("5b59aea3d49a2ca740134a43d6a0259c"))
        ]
        self._productTables = []
        ...
        h_table1 = [
            bytesToNumber(bytes.fromhex("e440c097e31a936a0000000000000000")),
            bytesToNumber(bytes.fromhex("ffa5f9409bf15bb00000000000000000")),
            bytesToNumber(bytes.fromhex("68009948c9489f960000000000000000")),
            bytesToNumber(bytes.fromhex("c10f1f4418e718430000000000000000")),
            bytesToNumber(bytes.fromhex("5b59aea3d49a2ca70000000000000000"))
        ]
        self._productTables1 = []
        for i in range(len(h_table1)):
            self._productTables1.append([0] * 16)
            self._genProductTable(self._productTables1[i], h_table1[i])

        h_table2 = [
            bytesToNumber(bytes.fromhex("000000000000000058c057325e7978ae")),
            bytesToNumber(bytes.fromhex("0000000000000000831967be333fc74f")),
            bytesToNumber(bytes.fromhex("000000000000000043d26dbb4e0a2005")),
            bytesToNumber(bytes.fromhex("0000000000000000df1333422fff6694")),
            bytesToNumber(bytes.fromhex("000000000000000040134a43d6a0259c"))
        ]
        self._productTables2 = []
        for i in range(len(h_table2)):
            self._productTables2.append([0] * 16)
            self._genProductTable(self._productTables2[i], h_table2[i])
    ...
    def _mul(self, y, productTable):
        ...
        for i in range(0, 128, 1):
            retHigh = ret_copy & 1
            ret_copy >>= 1
            ret_copy ^= (AESGCM._gcmReductionTableAlt[retHigh] << (128-8))
